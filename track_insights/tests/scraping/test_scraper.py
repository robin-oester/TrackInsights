...
from track_insights.scraping import ScrapeConfig, Scraper, BestlistField
...
def test_extract_data(parsed_bestlist_mock: MagicMock):
    config = get_sample_config()
    scraper = Scraper(config)
    with open(DF_PATH, "r") as sample_file:
    ...
    check_array_equality(dataframe.columns, ["Nr", "Resultat", "Wind", "Rang", "Name", "Verein",
                                             "Nat.", "Geb. Dat.", "Wettkampf", "Ort", "Datum", "athlete_code",
                                             "club_code", "event_code"])
    ...
    for idx, val in enumerate(zip(dataframe[BestlistField.ATHLETE.value], dataframe[BestlistField.CLUB.value],
                                  dataframe[BestlistField.EVENT.value], dataframe[BestlistField.LOCATION.value],
                                  dataframe[BestlistField.BIRTHDATE.value], dataframe[BestlistField.DATE.value])):
